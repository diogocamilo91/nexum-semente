...
3. **Duas portas de conversa** — o **Telegram** (o bolso dela) e o **chat web** (a mesa
   de trabalho: áudio, foto, print, anexo, arquivar).
...
| 4 · chat | **tudo pronto: `instalar.sh` faz sozinho** | rodar 1 comando e conferir |
...
### 4. CHAT WEB — `base/chat/`

Siga `base/chat/LEIA-ME.md`. **É um comando** (`bash base/chat/instalar.sh`): ele monta o
serviço, põe num endereço com HTTPS e prova sozinho. No fim, peça pra ela abrir no
celular, entrar e te mandar um "oi" — e adicionar à tela de início.

Explique a diferença das duas portas: **Telegram é o bolso** (recado rápido, áudio na
rua); **o chat é a mesa** (assunto longo, print, anexo, ler com calma).
...
- "Você fala comigo por dois lugares: **Telegram** (cada tópico é uma conversa; `/new`
  zera) e o **chat** no endereço que te passei — lá dá pra mandar áudio, foto, print e
  anexo, arquivar e excluir conversa."
...
| `base/chat/` | **chat web pronto** (`instalar.sh` faz tudo) | etapa 4 |
...
- **Scripts instalados:** `~/semente-bin/` · logs em `~/semente-bin/log/`.
  O bot mora em `~/semente-bot/` e o chat em `~/semente-chat/`.
